...
function calculateDynamicTechStack(portfolio) {
  ...
  Object.entries(techCounts).forEach(([tech, count]) => {
    ...
    const category = Object.keys(techMap).find(
      key => key.toLowerCase() === normalizedTech
    ) ? techMap[Object.keys(techMap).find(
      key => key.toLowerCase() === normalizedTech
...
function runTests() {
  ...
  console.log(totalPercentage === 100 ? '✅ Perfect! Sums to 100%' : `❌ Error: Should be 100%, got ${totalPercentage}%`);
  ...
  Object.entries(techCounts).forEach(([tech, count]) => {
    ...
    if (!categoryAnalysis[category].find(item => item.name === tech)) {
      categoryAnalysis[category].push({ name: tech, count, percentage: Math.round((count / testPortfolio.length) * 100) });
    }
  });
